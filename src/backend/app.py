...
app.config['SQLALCHEMY_DATABASE_URI'] = f'sqlite:///{os.path.join(BASE_DIR, "grocery_app.db")}'
...
def load_label_encoder():
    ...
    if os.path.exists(label_encoder_path):
        ...
        initial_data = pd.read_csv(os.path.join(BASE_DIR, 'data.csv'))
...
def load_user_data(user_id):
    ...
    """
    user_file = os.path.join(BASE_DIR, 'user_data', f'{user_id}.joblib')
    try:
